...
class _TrackerPageState extends State<TrackerPage> {
  ...
  Future<void> _runDiagnosis() async {
    if (_isRunningDiagnosis) return;
    setState(() {
      _isRunningDiagnosis = true;
      _status = 'Running diagnosis...';
    });
    ...
    final issues = <String>[];
    if (!_permissions.locationServices) issues.add('location services');
    if (!_permissions.locationGranted) issues.add('location permission');
    if (!_permissions.notificationGranted) issues.add('notifications');

    final serviceState = _isTracking
        ? (_locationStreamStoppedMode ? 'stopped mode' : 'moving mode')
        : 'inactive';
    final summary = issues.isEmpty
        ? 'Ready: permissions ok, service $serviceState'
    ...
    _logDiagnostic('Diagnosis check', summary);
      ...
      children: [
        ...
        Card(
          child: ListTile(
            ...
            onTap: _isRunningDiagnosis ? null : _runDiagnosis,
          ),
        ),
        Card(
          child: ExpansionTile(
            ...
            subtitle: const Text('Backup and restore full local history'),
            children: [
              ListTile(
                leading: const Icon(Icons.backup_rounded),
                title: const Text('Export JSON backup'),
                subtitle: const Text('Saves app data and copies it.'),
              ...
              ListTile(
                ...
                subtitle: const Text('Restore from clipboard JSON.'),
...
class _StatsGrid extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final metrics = shift == null
        ? null
        : ActivityMetrics.fromShifts(
            [shift!],
            periodStart: shift!.startedAt,
            periodEnd: now,
          );
    final utilization = metrics?.activePercentage ?? 0;
    final completedOrders = shift?.completedOrderCount ?? 0;
    final activeRatio = (utilization / 100).clamp(0.0, 1.0);
    return Column(
      children: [
        GridView.count(
          crossAxisCount: 2,
          ...
          children: [
            _StatCard(
              ...
              value: formatDuration(
                metrics?.activeDeliveryTime ?? Duration.zero,
              ),
            ),
            _StatCard(
              label: 'Waiting',
              value: formatDuration(
                metrics?.waitingNoOrderTime ?? Duration.zero,
              ),
            ),
            _StatCard(
              label: 'Active distance',
              value:
                  '${((metrics?.activeDistanceMeters ?? 0) / 1000).toStringAsFixed(2)} km',
            ),
            _StatCard(
              label: completedOrders > 0 ? 'Completed' : 'Utilization',
              ...
              children: [
                Row(
                  children: [
                    ...
                    const Spacer(),
                    Text(
                      '${utilization.toStringAsFixed(0)}% active',
                      style: Theme.of(context).textTheme.labelMedium,
                    ),
                  ],
                ),
...
class Shift {
  ...
  int get completedOrderCount => deliveryTrips.fold<int>(
        0,
        (total, trip) =>
            total +
            trip.orders
                .where((order) => order.status == DeliveryOrderStatus.delivered)
                .length,
      );
